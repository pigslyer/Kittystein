...
#define FLOOFY_TEST_REGISTER(test_func_name, enable)																															\
	...
	void _##test_func_name##_register(void) { if (enable) floofy_test_function_full(test_func_name, _delight_string_make_relative_to_src(__FILE__), __LINE__, #test_func_name); }

#define FLOOFY_TEST_ASSERT(condition, error)														\
	floofy_test_assert_full(_delight_string_make_relative_to_src(__FILE__), __LINE__, condition, error)

#define FLOOFY_TEST_ASSERT_ARG(condition, error, ...)																\
	floofy_test_assert_full(_delight_string_make_relative_to_src(__FILE__), __LINE__, condition, error, __VA_ARGS__)
